...
function buildAncestryGraph(source_node, personTree, context) {
  const nodes = [];
  const edges = [];
  ...
  for (const parent of parents) {
    ...
    const parentNode = { 
      ...
      position: { x: 0, y }, 
      data: { person: parent, hasTopHandle: parent?.famc?.length, hasBottomHandle: true, context } };
    ...
    const ancestryGraph = buildAncestryGraph(parentNode, parent, context);
...
function buildDescendencyGraph(sourceNode, personTree, context) {
  const nodes = [];
  const edges = [];
  ...
  for (const family of families) {
    ...
    const spouseNode = { 
      ...
      data: { person: spouse, hasLeftHandle: true, context } };
    ...
    */
    const childrenData = children.map((child) => {
      const childNode = { 
        ...
        data: { person: child, hasTopHandle: true, hasRightHandle: child.families?.length, context } };
      ...
      const descendencyGraph = buildDescendencyGraph(childNode, child, context);
...
export function buildGraphModel(personTree, context) {
  ...
  const ancestryGraph = buildAncestryGraph(source_node, personTree, context);
  nodes.push(...ancestryGraph.nodes);
  edges.push(...ancestryGraph.edges);

  const descendencyGraph = buildDescendencyGraph(source_node, personTree, context);
