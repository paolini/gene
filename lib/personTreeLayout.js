...
function buildAncestryGraph(source_node, personTree) {
  ...
  for (const parent of parents) {
    ...
    const parentNode = { id: parent.id, type: 'personNode', position: { x: x, y }, data: { person: parent, hasTopHandle: true, hasBottomHandle: true } };
    nodes.push(parentNode);
    edges.push({ id: `${source_node.id}-${parent.id}`, source: source_node.id, target: parent.id, sourceHandle: 'top', targetHandle: 'bottom', type: 'smoothstep' });
...
export function buildGraphModel(personTree) {
  ...
  let nodes = [];
  let edges = [];
  const source_node = { id: personTree.id, position: { x: 0, y: 0 }, type: 'personNode', data: { person: personTree, hasTopHandle: true, hasBottomHandle: true } };
...
/*
  ...
  const semanticModel = buildSemanticTreeModel(personMap);
  const { personEntries, families } = semanticModel;
  const personNodes = personEntries.map((entry) => ({
    id: `${entry.person.id}`,
    type: 'personNode',
    position: { x: entry.x * PERSON_X_SPACING, y: entry.level * PERSON_Y_SPACING },
    width: NODE_WIDTH,
...
function alignRenderedFamilyNodes(renderedNodes, edges) {
  ...
  for (const node of renderedNodes) {
    ...
    const averageCenterY = spouseNodes.reduce((sum, spouseNode) => sum + ((spouseNode.position?.y) || 0) + (spouseNode.height || 0) / 2, 0) / spouseNodes.length;

    familyNode.position.y = averageCenterY - (familyNode.height || 0) / 2;

    if (spouseNodes.length === 2) {
      const leftSpouse = (spouseNodes[0].position?.x || 0) <= (spouseNodes[1].position?.x || 0) ? spouseNodes[0] : spouseNodes[1];
      const rightSpouse = (spouseNodes[0].position?.x || 0) > (spouseNodes[1].position?.x || 0) ? spouseNodes[0] : spouseNodes[1];
      const leftCenterX = (leftSpouse.position?.x || 0) + (leftSpouse.width || 0);
      const rightCenterX = (rightSpouse.position?.x || 0);
      ...
      familyNode.position.x = (spouseNode.position?.x || 0) + ((spouseNode.width || 0) - (familyNode.width || 0)) / 2;
    }
  }
