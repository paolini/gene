...
function buildAncestryGraph(source_node, personTree) {
  console.log('Building ancestry graph for source_node', source_node, 'personTree', personTree);
  const nodes = [];
  const edges = [];
  const parents = personTree.parents || [];
  const totalSize = computeSizeOfAncestryNodes(personTree);
  let x = source_node.position.x - PERSON_X_SPACING * (totalSize.width - 1) / 2;
  let y = source_node.position.y - PERSON_Y_SPACING;
  console.log('Total ancestry size', totalSize, 'initial x', x, 'y', y);
  for (const parent of parents) {
    const {width } = computeSizeOfAncestryNodes(parent);
    console.log('Parent', parent, 'ancestry size', { width, x, y });
    const parentNode = { id: parent.id, type: 'personNode', position: { x: x, y }, data: { person: parent } };
    nodes.push(parentNode);
    edges.push({ id: `${source_node.id}-${parent.id}`, source: source_node.id, target: parent.id, type: 'smoothstep' });
    const ancestryGraph = buildAncestryGraph(parentNode, parent);
    nodes.push(...ancestryGraph.nodes);
    edges.push(...ancestryGraph.edges);
    x += PERSON_X_SPACING * width;
  }
  return { nodes, edges };
}
...
export function buildGraphModel(personTree) {
  ...
  let nodes = [];
  let edges = [];
  const source_node = { id: personTree.id, position: { x: 0, y: 0 }, type: 'personNode', data: { person: personTree } };
